...
public class NoticeWriterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 업로드할 파일의 용량 설정 : 10Mbyte로 제한한다면
		int maxSize  = 1024 * 1024 * 10;
		
		//enctype="multipart/form-data"로 전송왔는지 확인
		...
		//MultipartRequset 객체를 생성함
		//객체 생성과 동시에 자동 파일이 업로드 됨
		...
		//전송은 파일의 이름 추출
		String originalFileName = mrequest.getFilesystemName("");
		...
		if(originalFileName != null) {
			...
			String renameFileName = sdf.format(originalFileName.lastIndexOf(".") + 1);
			...
			//파일 이름 바꾸기 실행함
			//이름 바꾸기가 실패할 경우에는 직접 바꾸기 처리함
			//직접 바꾸기는 원본 파일에 대한 복사본 파일을 만
