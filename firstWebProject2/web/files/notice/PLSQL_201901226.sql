...
-- 프로시저 (PROCEDURE)
-- PLSQL 구문을 저장하는 객체
-- 컴파일하여 실행할 수 있음 : 반복 사용 가능해짐, 함수의 형태를 가짐
...
-- 실습
-- DEPARTMENT 테이블을 복사한 DEPT_COPY 테이블 만들기
-- 만약에 존재하면 테이블 삭제하고 다시 만들기
CREATE TABLE DEPT_COPY
AS
SELECT * FROM DEPARTMENT;

DROP TABLE DEPT_COPY CASCADE CONSTRAINTS;
-- 부서번호를 전달 받아서 해당 부서를 삭제하는 프로시저 : DEL_DEPTID
...
-- 실습 : 사원 이름 전달 받아서 해당 직원 정보 삭제하는 프로시저 : DEL_ENAME
-- EMP_COPY 테이블 사용 : 존재하면 삭제하고 새로 만듬
...
IS
BEGIN
    DELETE FROM EMP_COPY WHERE VENAME = EMP_NAME;
    COMMIT;
    DBMS_OUTPUT.PUT_LINE(VENAME || '직원이 퇴사하였습니다.');
END;
/

EXECUTE DEL_NAME('&사원이름');
...
-- BODY는 인터페이스 상속 받아서 추상메소드 오버라이딩 처리한 구현부로 비교됨

-- 패키지 선언 : HEAD 영역
CREATE OR REPLACE PACKAGE PMEMBER
IS
    -- 패키지에 속한 프로시저와 함수 선언함
...
-- *************************************************************
-- 커서(CURSOR)
-- SELECT 쿼리문의 실행 결과 (RESULT SET)에 대한 결과행의 개숫가 여러 개일 때
...
-- 실습 : EMPCPY 테이블 INSERT가 수행이 되고 나면, 자동으로 실행되는
-- 트리거 작성 : TR_WELCOME
...
-- 실습 : 사원테이블에 사원정보가(EMPNO, ENAME, SAL) 입력되었을때 급여테이블(NO, EMPNO, SAL)에 그 사원에 대한 급여 정보를 자동으로
-- 입력하도록 트리거를 설계하시오.
-- TR_SALARY
...
-- 예를 들어, 입고된 상품의 수량이 15에서 10으로 변경하면, 상품 테이블의 재고수량이 25에서 20으로 변경되게 해야함
...
-- 3. 삭제 트리거
-- 입고 테이블에서 행을 삭제하면, 삭제된 행의 수량만큼 상품 테이블의 재고수량을 줄임
...
EXEC SP_PRO_INSERT('A002', 50, 3000);
