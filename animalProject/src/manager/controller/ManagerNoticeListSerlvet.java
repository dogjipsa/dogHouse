...
/**
 * Servlet implementation class ManagerNoticeListSerlvet
 */
@WebServlet("/mannotice")
public class ManagerNoticeListSerlvet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ManagerNoticeListSerlvet() {
        super();
	...
	if(opt == null) {
		...
		ArrayList<Notice> noticeList = mservice.searchNoticeList(listOpt);
								
		//한 페이지에 출력할 목록 갯수 지정
		int limit = 10;
		
		//현재 페이지에 출력할 목록 조회
		int listCount =  mservice.getNoticeListCount(listOpt);
		...
		if(noticeList.size() >= 0) {
			view = request.getRequestDispatcher("views/manager/managerNoticeListView.jsp");
